...
export const formatQuery = async state => {
  ...
  const json = await request('POST', '/api/format-sql', {
...
export const loadQueries = store => async state => {
  ...
  if (
    ...
    const json = await request('GET', '/api/queries');
    if (json.error) {
      Message.error(json.error);
    }
    store.setState({
...
export const clearQueries = () => ({ queries: [] });

export const deleteQuery = store => async (state, queryId) => {
  const { queries } = state;
  const filteredQueries = queries.filter(q => q._id !== queryId);
  store.setState({ queries: filteredQueries });
  const json = await request('DELETE', `/api/queries/${ queryId}`);
  if (json.error) {
    Message.error(json.error);
...
export const loadQuery = async (state, queryId) => {
  const { error, query } = await request('GET', `/api/queries/${queryId}`);
  if (error) {
    Message.error(error);
  }
  return {
...
export const runQuery = store => async state => {
  ...
  const { queryResult, error } = await request(
    'POST',
    '/api/query-result',
    postData
  );
...
export const saveQuery = store => async state => {
  ...
  if (query._id) {
    request('PUT', `/api/queries/${query._id}`, queryData).then(json => {
      ...
      const updatedQueries = queries.map(q => (q._id === query._id ? query : q));
      store.setState({
        isSaving: false,
        unsavedChanges: false,
        query,
    ...
    request('POST', '/api/queries', queryData).then(json => {
      const { error, query } = json;
      const { queries } = store.getState();
      if (error) {
        Message.error(error);
...
export const handleQuerySelectionChange = (state, selectedText) => ({ selectedText });
